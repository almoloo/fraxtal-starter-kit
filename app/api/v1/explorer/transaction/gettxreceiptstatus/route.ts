...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const res = await fetch(
    `${baseUrl}?module=transaction&action=gettxreceiptstatus&txhash=${txHash}&tag=latest&apikey=${apiKey}`
  );

  if (!res.ok) returnError("Failed to fetch transaction receipt status.", 500);

  const jsonRes = await res.json();

  if (jsonRes.status === "0" || jsonRes.isError !== undefined)
    returnError("Failed to fetch transaction receipt status.", 500);
