...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const res = await fetch(
    `${baseUrl}?module=account&action=txlist&address=${address}&tag=latest&apikey=${apiKey}${extraParams}`
  );

  if (!res.ok) returnError("Failed to fetch transactions list.", 500);

  const jsonRes = await res.json();

  if (jsonRes.status === "0")
    returnError("Failed to fetch transactions list.", 500);
