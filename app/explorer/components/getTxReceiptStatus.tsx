...
const GetTxReceiptStatus = () => {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (status.status === '') throw new Error('Invalid txHash');
			setResponse(status);
		} catch (error) {
	...
	return (
		...
		>
			<h2 className="font-medium">Check Transaction Receipt Status</h2>
			...
			{error && (
				<section className="alert red">
					<p>Failed to fetch transaction receipt status:</p>
